...
final class APICaller {
    ...
    public func createPlaylists(with name: String, completion: @escaping (Result<Void, Error>) -> Void) {
        getCurrentUserProfile { [weak self] result in
            switch result {
            case let .success(profile):
                self?.createRequest(with: URL(string: Constants.baseURL + "/users/\(profile.id)/playlists"), type: .POST) { [weak self] baseRequest in
    ...
    public func removeTrackFromPlaylists(track: AudioTrack, playlistID: String, completion: @escaping (Result<Void, Error>) -> Void) {
        createRequest(with: URL(string: Constants.baseURL + "/playlists/\(playlistID)/tracks"), type: .DELETE) { [weak self] baseRequest in
            var request = baseRequest
            let json: [String: Any] = [
                "tracks":[
                    [
